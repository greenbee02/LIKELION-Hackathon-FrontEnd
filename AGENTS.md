...
- **The switch governs what the backend has. What it does not have is always mock** — currently care services (`src/lib/care.ts`) and brand events (`src/lib/events.ts`), the two domains with no controller at all. Such a module **does not import `config.ts`**, and that absence is how it is marked: `grep -L "from './config'" src/lib/*.ts` counts what is still unbuilt. The switch chooses between mock and live; it does not choose whether a screen exists. Their types live in their own module, not `types.ts`, which is for what the backend actually sends.
