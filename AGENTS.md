...
- **A rewrite cannot dodge CORS, because it does not strip `Origin`.** A browser attaches `Origin` to a POST even when it is same-origin, and the backend's allow-list has no deploy origin, so login answers `403 Invalid CORS request` while GETs pass. The proxy is a **function** for that one reason: a function can drop the header. Delete it the day the backend adds the origin (`backend-open-items.md` §4).
