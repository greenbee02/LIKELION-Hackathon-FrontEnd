...
카카오톡 코드스캔 화면을 **레이아웃 참고용으로만** 봤다. 위에서 아래로 — 제목, 카메라, 안내 한 줄.
...
- **카메라만 거터를 넘어간다.** 페이지에 놓인 사진이 아니라 방을 향한 창이라, 라이브 장면
  둘레의 16pt 여백은 그 장면을 사진처럼 보이게 만든다. 제목과 안내는 거터 안에 있다.
...
- **조준 마크는 작다(56pt, 코너만).** 디코더는 프레임 전체를 읽으므로 큰 사각형은 없는 규칙을
  ...
  같은 이유다.
- 떠 있는 컨트롤(플래시 · 앨범)은 규약대로 **near-white 글라스**다. 라벨은 step 12.
- 참고 화면의 **X 닫기와 하단 3탭은 뺐다.** 탭 화면이라 닫을 대상이 없고, 하단 탭은 앱의
  전역 탭바와 중복된다. 제목도 중앙 정렬 대신 컬렉션 탭과 같은 리듬으로 왼쪽에 둔다 —
  X 가 없는 탭 화면에서 가운데 제목은 혼자 뜬다.
- 권한은 **버튼 뒤가 아니라 진입 시에 묻는다.** 스캔이라는 탭을 눌러서 온 화면에 "카메라 써도
  되나요?" 컨트롤을 또 두면 같은 질문을 두 번 하는 셈이다. `canAskAgain` 이 false 가 되면
  재요청 루프가 멈춘다. 권한이 없으면 조준 마크도 숨는다 — 겨눌 장면이 없는데 겨냥선을
  그리지 않는다.
