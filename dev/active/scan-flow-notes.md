...
수동 코드 입력(2026-08-19 제거)과 앨범 업로드(2026-08-19 추가 후 같은 날 제거) 모두 사용자
지시로 빠졌다. 화면에 남은 입력 경로는 카메라뿐이다.

**대가는 알고 있다:** React Native Web 과 시뮬레이터에는 카메라가 없으므로
**웹에서는 스캔 화면으로 카드를 발급할 수 없다.** 시연은 폰 기준이다.
(`expo-image-picker` 는 앨범과 함께 제거했다. 다시 필요해지면
`scanFromURLAsync` 에 웹 구현이 있어 그 경로만 되살리면 된다.)
