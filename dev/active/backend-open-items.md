...
## 14. 🟢 승인 디자인의 문구가 `NotBlank` 라 — 글자 없는 카드를 만들 수 없다

`POST /cards/{cardId}/customizations/layers` 의 `text.content` 가 `NotBlank` 이고 `text` 를
통째로 생략할 길도 없다. 그래서 배경과 테두리만 고르고 아무것도 새기지 않은 카드는 저장이
불가능하다 — 화면은 문구를 적기 전까지 저장 버튼을 잠가두는 것으로 그 제약을 대신하고 있다.

세 겹 중 한 겹만 고객의 것인데 그 한 겹이 필수인 셈이라, 하우스의 그림 그대로가 좋은 고객은
의미 없는 한 글자를 적게 된다.

**요청:** `text` 를 선택 항목으로 하거나 `content` 의 `NotBlank` 를 풀 것. 문구가 없으면
`frontLayers` 가 두 줄(`PRODUCT_BACKGROUND`, `BORDER`)로 오면 된다 — 프론트는 이미 겹의
개수를 세지 않고 `type` 으로 그린다.
