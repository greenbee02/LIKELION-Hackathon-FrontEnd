 ...
 * 가운데를 지킨 채 한 줄로 잘린다.
 *
 * **`Screen` 의 `header` 로 넘긴다 — `children` 의 첫 줄이 아니라.** 그쪽에 넣으면 스크롤
 * 콘텐츠의 일부라 위로 떠내려가고, 뒤로 나갈 문이 화면 밖으로 사라진다. 이유는 `Screen` 에
 * 적혀 있다.
 *
 * **탭의 24pt 제목과는 쓰임이 다르다.** 저쪽은 뒤로 갈 데가 없는 화면이 제 이름을 크게 적은
 * 것이고 (`(tabs)` 의 `내 컬렉션` · `리워드` · `My`), 여기는 한 겹 안쪽으로 들어온 화면의
 * 이름표다. 둘은 같은 `header` 슬롯에 서지만 한 화면이 둘 다 쓰면 이름이 두 번 적힌다.
 */
