...
```bash
bash "$(git rev-parse --show-toplevel)/.Codex/skills/dev-up/dev-up.sh"
```
...
When **Codex** runs this, launch it as a background task and report the printed
...
· `4` readiness timed out. On `3`/`4` it prints the tail of `.Codex/dev-up.log`
